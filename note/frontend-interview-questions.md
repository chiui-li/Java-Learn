...
本文档整理了常见的前端面试技术问题，包括原型设计、联调流程、浏览器兼容适配、数据保存以及 React 通信等内容。
...
#### 1️⃣ 接口约定先行（API First）
- 使用 Swagger/YApi/Apigateway 等工具定义好接口契约
- 前后端分离项目，前端可提前开发 Mock data 模拟服务层调用测试验证功能完成度
- WebSocket实时推送消息，即时展示 loading 动画避免长时间空转超时时间约定一致
...
- HTTPStatus响应标准化处理异常捕获机制完善日志记录功能模块间数据一致性校验保障业务逻辑正确性。
```
...
```javascript
// ✅ Polyfill + PostCSS/Autoprefixer自动注入脚本兼容旧版浏览器+ES6新特性降级方案提供回退样式使用媒体查询 (@media) 或 flex/gridfallback写法保证基础体验。
const div = document.querySelector('.container'); // ES6 选择器

if (!window.CSS.supports('display', 'flex')) { 
    // ❌不支持则用float替代布局方式适配IE10+旧版浏览器降级方案提供回退机制确保页面可用性核心逻辑优先保障功能完整性视觉样式做渐进增强处理。
}
```

### 📱 PC/移动多端适配：

- **REM/RP单位**（响应式）配合 `viewport`meta 标签实现流动布局设计
- **媒体查询 (Media Queries)**: 
    ```css
...
前端面试准备需要全面了解各个技术栈的知识体系，建议：
- **原型设计**掌握 Figma/Sketch/Axure等工具基本用法和交互设计规范
- **联调流程**熟悉 RESTful API规范、HTTP状态码、错误处理机制
- **兼容适配**了解 Polyfill 使用、媒体查询断点设置、渐进增强原则
- **数据存储**能区分不同存储方式的优劣，根据场景选择合适方案  
- **React通信**掌握 Context/Redux/Hooks等不同方案的适用边界

希望这份整理能帮助面试官和求职者在面试中更好地展示专业能力！
