...
class VideoResize {
  ...
  private onEditorClick = (e: MouseEvent) => {
    ...
    if (target.tagName === "VIDEO" && this.quill.root.contains(target)) {
      this.select(target as HTMLVideoElement);
    } else {
      this.deselect();
    }
  };

  private select(video: HTMLVideoElement) {
    this.deselect();
    this.activeVideo = video;

    const overlay = document.createElement("div");
    overlay.className = "ql-video-resize";
    ...
    handle.addEventListener(
      ...
      (e: MouseEvent) => {
        ...
        const onUp = () => {
          ...
          video.setAttribute("width", String(finalW));
